...
def propose_new_capability(existing: list[str]) -> str:
    ...
    prompt = (
        ...
        "It must be implementable as a single self-contained Python plugin file using "
        "only the standard library or already-installed packages (requests, openpyxl, "
        "psutil, numpy). It must not duplicate anything above.\n\n"
